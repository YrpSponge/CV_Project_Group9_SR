...
class DynamicDatasetManager:
    ...
    def get_batch(self, global_step):
        ...
        if self.args.train_cfr_only:
            target_stage = 0  # CFR-only: single dataset, no switching
        else:
            target_stage = 0
...
def main(args):
    ...
    bpp_param_names = [n for n in param_dict_bpp.keys() if "spynet" not in n and "flow_net" not in n]
    ...
    if args.train_cfr_only:
        # CFR-only: train only CFR backbone params (excluding flow_net/RAFT)
        print("=== CFR-only mode: freezing UNet/LoRA, training only CFR backbone ===")
        set_requires_grad(quality_params, False)
        set_requires_grad(consistency_params, False)
        set_requires_grad(base_params, False)
        set_requires_grad(bpp_params, True)
        print(f"CFR backbone trainable params: {len(bpp_params)}")
        for n in bpp_param_names:
            print(f"  trainable: {n}")
        net_osediff.set_train_cfr_only()
        param_groups_cfr = [
            {'params': bpp_params, 'name': 'cfr_backbone'},
        ]
        optimizer = torch.optim.AdamW(param_groups_cfr, lr=args.learning_rate,
    ...
    for epoch in range(0, args.num_training_epochs):
        ...
        for _ in range(epoch_steps):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                if args.train_cfr_only:
                    # CFR-only: always consistency stage, no CSD loss
                    lambda_l2 = args.lambda_l2
                    lambda_lpips = args.lambda_lpips
                    lambda_vsd = 0.0
                    lambda_consistency = args.lambda_consistency
                    stages = 0
                elif global_step <= args.quality_iter:
                    lambda_l2 = args.lambda_l2
                ...
                if args.train_cfr_only:
                    # CFR-only: keep frozen state from optimizer setup
                    pass
                elif stages == 1:
                    ...
                    net_osediff.module.set_train_quality()
                    ...
                    set_requires_grad(bpp_params, True)
                    set_requires_grad(base_params, True)
                    net_osediff.module.set_train_consistency()
